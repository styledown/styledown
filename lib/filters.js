...
exports.unpackExample = function (parent, examples, pre, highlight) {
  var Cheerio = require('cheerio');
  var code = parent.text();
  var blocks = parseCodeText(code);
  ...
  blocks.forEach(function(block){
    ...
    if (block.tag === 'example') {
      ...
      if(lang != 'pre') {
        ...
        var codeblock = "<pre class='"+pre('code')+"'>"+ Hljs.highlight(lang, block.code).value +"</pre>";
        var $block = Cheerio.load("<div class='"+pre('lang-'+lang)+ " " + pre('code') + "'>" + codeblock + "</div>");
      } else {
        var $block = Cheerio.load("<div class='"+pre('lang-'+lang)+ "'>" + block.code + "</div>");
      }
    ...
    examples.append($block.root());
  });

  parent.after(examples);
  parent.remove();
};
...
exports.isolateTextBlocks = function ($, pre) {
  ...
  $('.'+pre('block')).each(function() {
    ...
    var noExample = ($this.html().indexOf(pre('examples')) === -1);
    var noCode    = ($this.html().indexOf(pre('code')) === -1);
    if (noExample && noCode) return;

    var $first = $('h3', this);
    var $text = $first.nextUntil('.'+pre('examples')+', .'+pre('code'));
